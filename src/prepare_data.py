...
STOPWORDS_EN = {"of", "the", "and", "for", "in", "on", "to", "over", "from", "with", "by", "at", "as", "an"}
...
def match_abbr_from_end(abbr, words):
    """Сопоставление букв аббревиатуры со словами с конца (пропуская стоп-слова)."""
    ...
    while i >= 0 and j >= 0:
        ch = abbr_chars[i]
        word = words[j]
        if word.lower() in STOPWORDS_EN or word.upper() in STOPWORDS_RU:
            j -= 1
            continue
        if ch.islower():
            if word.lower() == ch:
...
def crop_definition_to_abbr(abbr, definition):
    ...
    if matched:
        return " ".join(matched)
    abbr_len = len(abbr)
    if abbr_len <= len(words):
        filtered_words = [w for w in words if w.lower() not in STOPWORDS_EN and w.upper() not in STOPWORDS_RU]
        if len(filtered_words) >= abbr_len:
            last_n_words = filtered_words[-abbr_len:]
            initials = "".join(w[0].upper() for w in last_n_words)
            if initials == abbr.upper():
                return " ".join(last_n_words)

    if abbr_len <= 3:
        for start in range(len(words) - abbr_len + 1):
            candidate_words = words[start:start+abbr_len]
            if all(w[0].upper() == abbr[i].upper() for i, w in enumerate(candidate_words)):
...
def pattern_p1_checked(text):
    ...
    for match in re.finditer(rf"\(({ABBR_PATTERN})\)", text):
        ...
        if before:
            ...
            if not definition:
                print(f"[SKIP P1] {abbr} <- '{definition_raw}'")
        ...
        if match:
            ...
            if definition_cropped:
                ...
                if words and all(len(w) == 1 for w in words):
                    print(f"[SKIP P2] {abbr} <- def='{definition_cropped}' (all single-letter words)")
                    continue
    ...
    for line in section.splitlines():
        ...
        if match:
            ...
            if definition_cropped:
                ...
                print(f"[SKIP P3] {abbr} <- def='{definition}' (crop failed)")
    return results
...
def pattern_p4_checked(text):
    ...
    for match in re.finditer(rf"({ABBR_PATTERN})\s*[\(（](.*?)[\)）]", text, re.DOTALL):
        ...
        if re.search(r'[^А-Яа-яЁёA-Za-z\s\-]', inner_clean):
            print(f"[SKIP P4] {abbr} <- inner='{inner_clean[:50]}...' (invalid chars)")
            continue
        words = re.findall(r"[А-Яа-яЁёA-Za-z\-]+", inner_clean)
        if len(words) < len(abbr):
            print(f"[SKIP P4] {abbr} <- words={words} (too few words)")
            continue
        matched = match_abbr_from_end(abbr, words)
        if matched:
            ...
            print(f"[SKIP P4] {abbr} <- words={words} (end-matching failed)")
    return results
...
def process_pdf_folder(folder_path, output_json="abbreviations.json"):
    ...
    for filename in os.listdir(folder_path):
        ...
        text = extract_pdf_text.extract_text(path)
